...
use optics_core::db::{DbError, DB};
use optics_core::{
    accumulator::merkle::Proof,
    traits::{
        ChainCommunicationError, Common, DoubleUpdate, Home, RawCommittedMessage, State, TxOutcome,
    },
    utils, Decode, Encode, Message, OpticsMessage, SignedUpdate, Update,
};
...
use std::{convert::TryFrom, error::Error as StdError, future::Future, sync::Arc};

use optics_core::db::iterator::PrefixIterator;
    ...
    M: ethers::providers::Middleware + 'static,
{
    ...
    pub fn new(name: &str, domain: u32, address: Address, provider: Arc<M>, db: DB) -> Self {
        Self {
            ...
            domain,
            name: name.to_owned(),
            db: HomeDB::new(db, name.to_owned()),
            provider,
        }
    }
}
