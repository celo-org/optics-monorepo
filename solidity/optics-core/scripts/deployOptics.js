...
async function deployOptics(local, remotes) {
  ...
  for (let remote of remotes) {
    ...
    await xAppConnectionManager.enrollReplica(replica.proxy.address, remoteDomain);

    // Add watcher permissions for Replica
    for (let watcher in watchers) {
      await xAppConnectionManager.setWatcherPermission(watcher, remoteDomain, true);
    }
  }
