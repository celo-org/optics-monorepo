...
describe('Common', async () => {
  ...
  let gasPrice: any; // TODO: define
  // let fakeUpdater = await optics.Updater.fromSigner(fakeSigner, localDomain);

  beforeEach(async () => {
    let commonFactory = new contracts.UpdaterManager__factory(deployer);
    common = await commonFactory.deploy(localDomain, updater.signer.address, { gasPrice });
